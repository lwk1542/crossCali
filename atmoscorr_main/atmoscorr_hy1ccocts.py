# -*- coding: utf-8 -*-
"""
@Time    : 2023/11/23 16:33
@FileName: atmoscorr_hy1ccocts.py
...
import os
import gc
from utils import esdist, resize, decomp
...
class Calcu(object):
    def __init__(self):
        ...
        self.filespath = r"C:\Users\DELL\Desktop\test"  # 数据路径
        self.nir_iter = True
    ...
    def run_main(self):
        ...
        for subdir in dirs:
            if ("H1C_OPER_OCT_L1A_2021" not in subdir) or ("_10.tar.gz" not in subdir):
                continue
            _ = self.filespath + os.sep + subdir
            self.infile = decomp.targz_hy1(_)
    ...
    def cloud_land_mask(self, rhos):
        """
        """
        if self.sensorID == "hy1ccocts":
            from sensor.hy1ccocts import mask
            _ = mask.cloud_land_mask(rhos=rhos)
        return _
