# -*- coding: utf-8 -*-
"""
@Time    : 2023/10/25 11:26
@FileName: atmoscorr_sdgsat1miiV2.py
...
class Calcu(object):
    def __init__(self):
        ...
        self.nir_iter = True
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        self.sensor_alt = sensor_extinfo.satellite_alt(sensor_id=self.sensorID)
    ...
    def run_main(self):
        ...
        files = file_identify.infile_list(path=self.filespath, sensor=self.sensorID)
        for self.infile in files:
            ...
            for self.block_num, d_i_temp in enumerate(self.data_Iterator):  # 每个传感器的迭代器数据内容可能不一样，针对性处理
                ...
                Lt = (data * gains.reshape((1, 1, -1)) + offsets.reshape((1, 1, -1)))
                ...
                if self.outfile.endswith(".H5"):
                    ...
                    for i, _temp in enumerate([self.lon, self.lat, self.vza, self.vaa, self.sza, self.saa]):
                        output.write(ds_group=self.navi_group, data_name=navi_para[i],
                                     chunk=_temp, rows_ext=self.rows_ext, columns=self.columns_img)
                    del _temp, navi_para
                ...
                if self.rrs_out:
                    ...
                    output.write(ds_group=self.geo_group, data_name="chlor_a", chunk=chl, rows_ext=self.rows_ext,
    ...
    def out_varable(self):
        self.outfile = os.path.splitext(self.infile)[0] + "_L2_1116.H5"
        self.ds, self.navi_group, self.geo_group = output.create(outfile=self.outfile)
    ...
    def unit(self, lt):
        ...
        if self.sensorID in ["olcis3a", "olcis3b"]:
            lt = lt / 10.
        elif self.sensorID in ["sdgsat1mii"]:
            lt = lt * 1.
        else:
            lt = lt * 1.
        return lt
