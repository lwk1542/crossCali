...
class SensorInfo(object):
    ...
    def read_aw_bw(self):
        ...
        Returns:
        """
        aw_bw_file = r'D:\researchProject_lwk\atmospheric_correction\oceancolor_acnirv2\RSR/' \
                     r'Absorption_and_scattering_coefficients_for_water.txt'
        f_reference = open(aw_bw_file, "r")
        lines = f_reference.readlines()[:]
        wavelength = [float(i.split(" ", -1)[0]) for i in lines]
        aw = [float(i.split(" ", -1)[1]) for i in lines]
        bw = [float(i.split(" ", -1)[2]) for i in lines]
        aw_ = pd.DataFrame({"wavelength": wavelength,
                            "aw": aw
                            })
        bw_ = pd.DataFrame({"wavelength": wavelength,
                            "aw": bw
                            })
        # target_rsr = pd.read_excel(io=rsr_infile, sheet_name=sensorid, header=0, index_col=None)
        # aw_ = assistant.calculate_band_average(reference_spectrum=aw_df, spectrum_response_function=target_rsr)
        # bw_ = assistant.calculate_band_average(reference_spectrum=bw_df, spectrum_response_function=target_rsr)
    ...
    def write_target(self):
        ...
        aw_ref, bbw_ref = self.read_hico()
        aw_ref, bbw_ref = aw_bbw()
        aw_ref, bbw_ref = self.read_aw_bw()
        aw_bands = calculate_band_average(reference_spectrum=aw_ref, spectrum_response_function=target_rsr)
        ...
        bbw_bands = calculate_band_average(reference_spectrum=bbw_ref, spectrum_response_function=target_rsr)
...
def aw_bbw():
    ...
    file = r"D:\researchProject_lwk\atmospheric_correction\oceancolor_acnirv2\RSR/water_spectra.nc"
    ds = nc.Dataset(file, mode="r")
    wl = ds["wavelength"][:]
    aw = ds["aw"][:]
    bbw = ds["bw"][:]

    aw_ = pd.DataFrame({"wavelength": wl,
                        "value": aw
                        })
    bbw_ = pd.DataFrame({"wavelength": wl,
                         "value": bbw
                         })
    return aw_, bbw_
...
def read_aw_bw():
    ...
    wavelength = [float(i.split(" ", -1)[0]) for i in lines]
    aw = [float(i.split(" ", -1)[1]) for i in lines]
    bw = [float(i.split(" ", -1)[1]) for i in lines]
    aw_ = pd.DataFrame({"wavelength": wavelength,
                          "aw": aw
                          })
    bw_ = pd.DataFrame({"wavelength": wavelength,
