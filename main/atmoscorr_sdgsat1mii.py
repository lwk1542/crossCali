...
class Calcu(object):
    ...
    def run_main(self):
        ...
        for subdir in dirs:
            ...
            for self.block_num, (data, self.lon, self.lat) in enumerate(self.data_Iterator):
                ...
                self.Lt = data.transpose(1, 2, 0) * self.gains.reshape((1, 1, -1))*1. + self.bias.reshape((1, 1, -1))
                ...
                if self.rrc_out:
                    ...
                    Rrc = np.pi * self.Ltemp / self.Fo_ / self.tg_sol / self.t_sol / np.cos(
                        np.deg2rad(self.sza_.reshape((self.rows_org, self.columns_org, 1))))
    ...
    def resampe(self):
        ...
        print(self.Lt[:, :, 0].shape)
        self.sza_ = np.full_like(self.Lt[:, :, 0], fill_value=self.sza)
        self.vza_ = np.full_like(self.sza_, fill_value=self.vza)
        self.saa_ = np.full_like(self.sza_, fill_value=self.saa)
        self.vaa_ = np.full_like(self.sza_, fill_value=self.vaa)
        ...
        self.sza_upscale = skimage.measure.block_reduce(self.sza_, block_size=(self.block_size, self.block_size),
                                                        func=np.nanmean, cval=np.nan, func_kwargs={'dtype': self.dtype})
        self.vza_upscale = skimage.measure.block_reduce(self.vza_, block_size=(self.block_size, self.block_size),
                                                        func=np.nanmean, cval=np.nan, func_kwargs={'dtype': self.dtype})
        self.saa_upscale = skimage.measure.block_reduce(self.saa_, block_size=(self.block_size, self.block_size),
                                                        func=np.nanmean, cval=np.nan, func_kwargs={'dtype': self.dtype})
        self.vaa_upscale = skimage.measure.block_reduce(self.vaa_, block_size=(self.block_size, self.block_size),
                                                        func=np.nanmean, cval=np.nan, func_kwargs={'dtype': self.dtype})
