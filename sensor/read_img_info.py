...
def get(infile=None, sensor_id=None, block_size: int = None):
    ...
    """
    # python 3.10后的语法可用match ... case ...
    # match sensor_id:
    #     case "sdgsat1mii":

    if sensor_id == "sdgsat1mii":  # 传感器
        if all(idx in os.path.basename(infile) for idx in ['KX10_MII_', "L4B"]):  # 数据级别
            from sensor.sdgsat1mii import read_img
    elif sensor_id == 'olcis3a' or sensor_id == 'olcis3b':
        if all(idx in os.path.basename(infile) for idx in ['OL_1_EFR____', ".SEN3"]):  # 数据级别
            from sensor.sentinel3 import read_img
    elif sensor_id == 'hy1acocts':
        pass
    elif sensor_id == 'hy1bcocts':
        pass
    elif sensor_id == 'hy1ccocts':
        if all(idx in os.path.basename(infile) for idx in ['H1C_OPER_OCT_L1A_', "_10.h5"]):  # 数据级别
    ...
    # [data_Iterator, year, month, day, num_443, num_490, num_520, num_555,
    #  num_670, nirs_num, nirl_num, nwvis, red, rows, columns] = image_info  # 如果数据正常data_Iterator是一个迭代器，包含有多个参数
    return image_info
