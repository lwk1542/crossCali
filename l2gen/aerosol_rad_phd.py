...
@file: aerosol_rad_phd.py
@time: 2022/2/18 22:48
@desc:
"""
# -*- coding: utf-8 -*-
"""
...
import predefine
from l2gen import airmass, array_simplify
...
def model_phase(aermod_index=None, sza=None, saa=None, vza=None, vaa=None, reaa=None, band: float = None):
    """
    Parameters
    ----------
    aer_models : 气溶胶模型
    ...
    fres1 = fresnel_coef(angle=vza, index=nw)
    fres2 = fresnel_coef(angle=sza, index=nw)
    phase_funcs = aermod_interp_func('wave_lut', 'scatt_lut', target_value='phase_lut')

    bands = band.repeat(sza.size).reshape(1, -1)
    points1 = np.array([bands, scatt1]).T
    points1 = points1[:, 0, :]
    points2 = np.array([bands, scatt2]).T
    points2 = points2[:, 0, :]

    phase1 = np.zeros(shape=(1, sza.size))
    phase2 = np.zeros(shape=(1, sza.size))
...
def fresnel_coef(angle=None, index=None):
    # print('fresnel_coef')
    mu = np.cos(angle * np.pi / 180)
    # float sq, r2, q1;
...
def model_epsilon(aermod_index=None, sza=None, saa=None, vza=None, vaa=None, bands=np.array([750, 865])):
    """
    ...
    bands2 = bands[1].repeat(sza.size).reshape(1, -1)
    points2 = np.array([bands2]).T
    points2 = points2[:, 0, :]

    bands1 = bands[0].repeat(sza.size).reshape(1, -1)
    points1 = np.array([bands1]).T
    points1 = points1[:, 0, :]

    albedo2 = np.zeros(shape=(1, sza.size))
    extc2 = np.zeros(shape=(1, sza.size))
    albedo1 = np.zeros(shape=(1, sza.size))
    extc1 = np.zeros(shape=(1, sza.size))
    for index in np.unique(aermod_index):
        # 把同一个模型的反演出来
        loc = np.argwhere(aermod_index == index)[:, 1]
        pts2 = points2[loc, :]
    ...
    phase2 = model_phase(aermod_index=aermod_index, sza=sza, saa=saa, vza=vza, vaa=vaa, band=bands[1])
    phase2 = phase2.reshape(1, -1)
    rhoas2 = phase2 * albedo2 * extc2

    # 2. 目标波段的理论模型rhoas
    phase1 = model_phase(aermod_index=aermod_index, sza=sza, saa=saa, vza=vza, vaa=vaa, band=bands[0])
...
def rh_select_models(relative_humidity=None):
    """
    根据湿度选择20个模型
    Parameters
    ----------
...
def model_taua(aermod_index=None, sza=None, saa=None, vza=None, vaa=None, reaa=None, rhoas_nir2=None, nir_l_wave: float = None):
    """
    ...
    phase = model_phase(aermod_index=aermod_index, sza=sza, saa=saa, vza=vza, vaa=vaa,reaa=reaa, band=nir_l_wave)

    albedo_funcs = aermod_interp_func('wave_lut', target_value='albedo_lut')
    bands = nir_l_wave.repeat(sza.size).reshape(1, -1)
    points = np.array([bands]).T
    points = points[:, 0, :]

    albedo = np.zeros(shape=(1, sza.size))
    for index in np.unique(aermod_index):
        # 把同一个模型的反演出来
        loc = np.argwhere(aermod_index == index)[:, 1]
        pts = points[loc, :]
    ...
    aot_nir2 = rhoas_nir2 * 4. * np.cos(sza * np.pi / 180) * np.cos(vza * np.pi / 180) / albedo / phase
...
def rhoa_to_rhoas(aermod_index: np.ndarray = None, rho_a=None, sza=None, vza=None, saa=None, vaa=None, band=None,
                  aeroob=1):
    """
    ...
    bands = band.repeat(sza.size).reshape(1, -1)
    points = np.array([bands, sza, reaa, vza]).T
    points = points[:, 0, :]
    # aermod_index=np.array([26, 26, 13, 18, 15, 55, 45, 13, 18, 55])
    # aermod_index=aermod_index.reshape(1,-1)
    a = np.zeros(shape=(1, sza.size))
    b = np.zeros(shape=(1, sza.size))
    c = np.zeros(shape=(1, sza.size))
    for index in np.unique(aermod_index):
        # 把同一个模型的反演出来
        loc = np.argwhere(aermod_index == index)[:, 1]
        pts = points[loc, :]
...
def rhoas_to_rhoa(aermod_index: np.ndarray = None, rhoas=None, sza=None, vza=None, saa=None, vaa=None, reaa=None,
                  band=None, aeroob=1):
    """
...
def model_transmittance(aermod_index: np.ndarray = None, sza=None, vza=None, bands=None, aot=None):
    """
    bands: 所有波长
    aot: 所有波段的aot
    aot和bands应该保持相同的
    ...
    a0 = np.zeros(shape=(bands.size, sza.size))
    b0 = np.zeros(shape=(bands.size, sza.size))
    a = np.zeros(shape=(bands.size, sza.size))
    b = np.zeros(shape=(bands.size, sza.size))
    points1 = np.zeros(shape=(sza.size, 2))
    points1[:, 1] = sza
    points2 = np.zeros(shape=(vza.size, 2))
    points2[:, 1] = vza
...
def diff_tran(aer_model_min=None, aer_model_max=None, delta=None, sza=None, vza=None,
              pressure=None, tauamin=None, tauamax=None, bands=None, taur=None):
    """
    :param aer_model_min:
    :param aer_model_max:
    :param delta:
    ...
    t_sensor_low, t_solar_low = model_transmittance(aermod_index=aer_model_min, sza=sza, vza=vza, bands=bands,
                                                    aot=tauamin)
    t_sensor_up, t_solar_up = model_transmittance(aermod_index=aer_model_max, sza=sza, vza=vza, bands=bands,
                                                  aot=tauamax)
    ...
    t_sensor = t_sensor * np.exp(-0.5 * taur / np.cos(sza * np.pi / 180) * (pressure / 1013.25 - 1))
    t_solar = t_solar * np.exp(-0.5 * taur / np.cos(vza * np.pi / 180) * (pressure / 1013.25 - 1))

    # 大气路径校正
    airmass_sph = airmass.ky_airmass(sza) + airmass.ky_airmass(vza)
    airmass_plp = airmass.pp_airmass(sza) + airmass.pp_airmass(vza)
...
def wangaer(bands=None, sza=None, vza=None, saa=None, vaa=None, aer_model_min=None, aer_model_max=None,
            rhoa_nir2=None, delta=None, nirl_num=None):
    """
    ...
    rhoasmin_nir2 = rhoa_to_rhoas(aermod_index=aer_model_min, rho_a=rhoa_nir2, sza=sza, vza=vza, saa=saa, vaa=vaa,
                                  band=bands[nirl_num], aeroob=1)
    rhoasmax_nir2 = rhoa_to_rhoas(aermod_index=aer_model_max, rho_a=rhoa_nir2, sza=sza, vza=vza, saa=saa, vaa=vaa,
                                  band=bands[nirl_num], aeroob=1)

    # 1 根据确定的模型计算各各波段的epsilon->气溶胶单次散射->气溶胶多次散射
    rhoa = np.full(shape=(bands.size, sza.shape[1]), fill_value=np.nan)
    for num, band in enumerate(bands):
        epsmax = model_epsilon(aermod_index=aer_model_max, sza=sza, saa=saa, vza=vza, vaa=vaa,
                               bands=np.array([band, bands[nirl_num]]))
        epsmin = model_epsilon(aermod_index=aer_model_min, sza=sza, saa=saa, vza=vza, vaa=vaa,
                               bands=np.array([band, bands[nirl_num]]))
        ...
        rhoamax = rhoas_to_rhoa(aermod_index=aer_model_max, rhoas=rhoasmax, sza=sza, vza=vza, saa=saa, vaa=vaa,
                                band=np.array([band]),
                                aeroob=1)
        rhoamin = rhoas_to_rhoa(aermod_index=aer_model_min, rhoas=rhoasmin, sza=sza, vza=vza, saa=saa, vaa=vaa,
                                band=np.array([band]),
                                aeroob=1)
...
def load_aermod(aerosol_lut_filepath=r'D:\COCTS_LUTs\Modis_LUT\modisa\aerosol'):
    # loads the entire aerosol model table for the specified model list
    # 把全部80个模型全部读入内存，以便之后快速筛选

    models = ["c50", "c70", "c90", "c99", "m50", "m70", "m90", "m99", "t50", "t90", "t99", "o99"]

    aerosol_models = {}
    for i in range(models.__len__()):
        # print(os.path.abspath(glob.glob(aerosol_lut_filepath + os.sep + 'aerosol*' + models[i] + '*.hdf')[0]))
        aerofile=glob.glob(aerosol_lut_filepath + os.sep + 'aerosol*' + models[i] + '*.hdf')[0]
        aerosol_model = {}
        aerosol_model['name'] = i
        aero_paras = ['wave', 'scatt', 'albedo', 'extc', 'angstrom', 'phase', 'solz', 'senz', 'phi', 'acost', 'bcost',
                      'ccost', 'dtran_wave', 'dtran_theta', 'dtran_a', 'dtran_b', 'dtran_a0', 'dtran_b0']
        try:
            f = SD(aerofile, SDC.READ)
            for aero_para in aero_paras:
                # GW94查找表不区分上下透过率
                if aero_para == 'dtran_a0':
                    aerosol_model[aero_para + '_lut'] = (f.select('dtran_a')).get()
                elif aero_para == 'dtran_b0':
                    aerosol_model[aero_para + '_lut'] = (f.select('dtran_b')).get()
                elif aero_para == 'phase':
                    aerosol_model[aero_para + '_lut'] = np.log((f.select(aero_para)).get())
                else:
                    aerosol_model[aero_para + '_lut'] = (f.select(aero_para)).get()

        except:
            f = nc.Dataset(aerofile, mode='r')
            for aero_para in aero_paras:
                if aero_para == 'phase':
...
def model_select_mobley(rho_a_nir1=None, rho_a_nir2=None, sza=None, vza=None, saa=None, vaa=None,
                        relative_humidity=None, aerosol_models=None, bands=np.array([750, 865])):
    """
    ...
    rh_models = rh_select_models(relative_humidity=relative_humidity, aerosol_models=aerosol_models)

    # 第二步：计算两个近红外波段的模型epsilon
    # 为了计算理解和函数通用，逐行计算，因此这里要循环20次
    epsilon_model_rh = np.zeros(shape=(20, sza.shape[1]))
    for model_i in range(20):
        epsilon_model_rh[model_i, :] = model_epsilon(aer_models=rh_models[model_i, :], bands=bands,
...
def model_select_wang(relative_humidity=None, sza=None, saa=None, vza=None, vaa=None, rho_a_nir1=None, rho_a_nir2=None,
    ...
    eps_retri = np.zeros(shape=(20, sza.shape[1]))
    epsilon_model_rh = np.zeros(shape=(20, sza.shape[1]))
    for i in range(20):
        if i < 10:
        ...
        rhoas_nirs_retri = rhoa_to_rhoas(aermod_index=aermod_index, rho_a=rho_a_nir1, sza=sza, vza=vza, saa=saa,
                                         vaa=vaa, band=nir_bands[0], aeroob=1)
        rhoas_nirl_retri = rhoa_to_rhoas(aermod_index=aermod_index, rho_a=rho_a_nir2, sza=sza, vza=vza, saa=saa,
                                         vaa=vaa, band=nir_bands[1], aeroob=1)
        eps_retri_temp = rhoas_nirs_retri / rhoas_nirl_retri
        eps_retri[i, :] = eps_retri_temp

        epsilon_model_rh[i, :] = model_epsilon(aermod_index=aermod_index, bands=nir_bands, sza=sza, saa=saa, vza=vza,
                                               vaa=vaa)

    eps_ave = np.nanmean(eps_retri, axis=0).reshape(1, -1)
    column_idx = np.arange(0, sza.shape[1])
    for i in range(8):
    ...
    # 最终的模型指数
    aer_model_min = np.zeros(shape=(1, sza.size))
...
def cross_calibration(delta=None, rhoa_nirl=None, sza=None, vza=None, saa=None, vaa=None, aer_model_max=None,
                      aer_model_min=None, aerosol_lut_filepath=None, F0=None, pressure=None, taur=None,
    ...
    rhoas_nirl_min_ref = rhoa_to_rhoas(aermod_index=aer_model_minx, rho_a=rhoa_nirlx, sza=sza_refx, vza=vza_refx,
                                       saa=saa_refx, vaa=vaa_refx, band=bands[nirl_num], aeroob=1)
    rhoas_nirl_max_ref = rhoa_to_rhoas(aermod_index=aer_model_maxx, rho_a=rhoa_nirlx, sza=sza_refx, vza=vza_refx,
                                       saa=saa_refx, vaa=vaa_refx, band=bands[nirl_num], aeroob=1)

    # 单次散射转换
    phase_max_ref = model_phase(aermod_index=aer_model_maxx, sza=sza_refx, saa=saa_refx, vza=vza_refx, vaa=vaa_refx,
                                band=bands[nirl_num])
    phase_max_tar = model_phase(aermod_index=aer_model_maxx, sza=szax, saa=saax, vza=vzax, vaa=vaax,
                                band=bands[nirl_num])
    rhoas_nirl_max_tar = rhoas_nirl_max_ref * phase_max_tar * np.cos(sza_refx * np.pi / 180) * np.cos(
        vza_refx * np.pi / 180) / phase_max_ref / np.cos(szax * np.pi / 180) / np.cos(vzax * np.pi / 180)

    phase_min_ref = model_phase(aermod_index=aer_model_minx, sza=sza_refx, saa=saa_refx, vza=vza_refx, vaa=vaa_refx,
                                band=bands[nirl_num])
    phase_min_tar = model_phase(aermod_index=aer_model_minx, sza=szax, saa=saax, vza=vzax, vaa=vaax,
                                band=bands[nirl_num])
    rhoas_nirl_min_tar = rhoas_nirl_min_ref * phase_min_tar * np.cos(sza_refx * np.pi / 180) * np.cos(
        vza_refx * np.pi / 180) / phase_min_ref / np.cos(szax * np.pi / 180) / np.cos(vzax * np.pi / 180)

    # 计算出模型里所有波长的taua，在这里波段给一个nir_l
    taua_max = model_taua(aermod_index=aer_model_maxx, sza=szax, saa=saax, vza=vzax, vaa=vaax,
                          rhoas_nir2=rhoas_nirl_max_ref,
                          nir_l_wave=bands[nirl_num])

    taua_min = model_taua(aermod_index=aer_model_minx, sza=szax, saa=saax, vza=vzax, vaa=vaax,
                          rhoas_nir2=rhoas_nirl_max_ref,
                          nir_l_wave=bands[nirl_num])

    taua, t_sensor, t_solar = diff_tran(aer_model_min=aer_model_minx, aer_model_max=aer_model_maxx,
                                        delta=deltax, sza=szax, vza=vzax, pressure=pressurex,
                                        bands=bands, tauamin=taua_min, tauamax=taua_max, taur=taur)

    # 1 根据确定的模型计算各各波段的epsilon->气溶胶单次散射->气溶胶多次散射
    rhoa = np.full(shape=(bands.size, szax.shape[1]), fill_value=np.nan)
    for num, band in enumerate(bands):
        epsmax = model_epsilon(aermod_index=aer_model_maxx, sza=szax, saa=saax, vza=vzax, vaa=vaax,
                               bands=np.array([band, bands[nirl_num]]))
        epsmin = model_epsilon(aermod_index=aer_model_minx, sza=szax, saa=saax, vza=vzax, vaa=vaax,
                               bands=np.array([band, bands[nirl_num]]))
        ...
        # /* compute MS aerosol reflectance in visible bands */
        rhoamax = rhoas_to_rhoa(aermod_index=aer_model_maxx, rhoas=rhoasmax, sza=szax, vza=vzax, saa=saax, vaa=vaax,
                                band=band, aeroob=1)
        rhoamin = rhoas_to_rhoa(aermod_index=aer_model_minx, rhoas=rhoasmin, sza=szax, vza=vzax, saa=saax, vaa=vaax,
                                band=band, aeroob=1)
        # /* interpolate between upper and lower-bounding models */
    ...
    return La_part1, t_[:, :, 0:bands.size], t_[:, :, bands.size:bands.size * 2], t_[:, :,
                                                                                  bands.size * 2:bands.size * 3]
...
def aerosol(rho_a_nir1=None, rho_a_nir2=None, sza=None, vza=None, saa=None, vaa=None, relative_humidity=None,
            aerosol_lut_filepath=None, bands=None, pressure=None, nirs_num: int = None, nirl_num: int = None,
            taur=None):
    # 1读取查找表
    ...
    aer_model_min, aer_model_max, epsilonlow, epsilonup, delta = \
        model_select_wang(relative_humidity=relative_humidity, sza=sza, saa=saa, vza=vza, vaa=vaa,
                          rho_a_nir1=rho_a_nir1, rho_a_nir2=rho_a_nir2,
                          nir_bands=np.array([bands[nirs_num], bands[nirl_num]]))

    print('====computing aerosol reflectance ...')
    # 计算各波段的气溶胶辐射
    rhoa, rhoasmin_nir2, rhoasmax_nir2 = wangaer(sza=sza, vza=vza, saa=saa, vaa=vaa, delta=delta, rhoa_nir2=rho_a_nir2,
                                                 aer_model_min=aer_model_min, aer_model_max=aer_model_max,
                                                 bands=bands, nirl_num=nirl_num)
    print('====computing taua/t_sen/tsol ...')
    # 计算出模型里所有波长的taua，在这里波段给一个nir_l
    taua_max = model_taua(aermod_index=aer_model_max, sza=sza, saa=saa, vza=vza, vaa=vaa, rhoas_nir2=rhoasmax_nir2,
                          nir_l_wave=bands[nirl_num])
    taua_min = model_taua(aermod_index=aer_model_min, sza=sza, saa=saa, vza=vza, vaa=vaa, rhoas_nir2=rhoasmin_nir2,
                          nir_l_wave=bands[nirl_num])
...
def calculate(bands=np.array([414., 444, 491, 519, 568, 671, 751, 862]), l_a_nir1=None, l_a_nir2=None, lon=None,
              lat=None, F0=None, sza=None, saa=None, vza=None, vaa=None, nirs_num: int = None, nirl_num: int = None,
              aerosol_lut_filepath=None, winds_peed=None, pressure=None, relative_humidity=None, taur=None):
    # nir1表示短波长的近红外波段，nir2表示长波长的近红外波段

    # 福亮度转反射率
    rhoa1 = l_a_nir1 * np.pi / F0[nirs_num] / np.cos(sza * np.pi / 180)
    rhoa1[np.isnan(rhoa1)] = -999
    rhoa2 = l_a_nir2 * np.pi / F0[nirl_num] / np.cos(sza * np.pi / 180)
    rhoa2[np.isnan(rhoa2)] = -999
    ...
    if rhoa2x.size < predefine.thresholds().pixels_num:
        return None
    rhoa, taua, t_sensor, t_solar, aer_model_min, aer_model_max, delta, rhoasmin_nir2, rhoasmax_nir2 = aerosol(
        rho_a_nir1=rhoa1x, rho_a_nir2=rhoa2x, pressure=pressurex,
        sza=szax, vza=vzax, saa=saax, vaa=vaax,
        relative_humidity=relative_humidityx,
        aerosol_lut_filepath=aerosol_lut_filepath,
        bands=bands, nirs_num=nirs_num, nirl_num=nirl_num, taur=taur)
    # print(aermod_low)
