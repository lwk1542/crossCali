...
def decomp(infile):
    ...
    tif_a = outpath + os.sep + os.path.basename(outpath) + "_A.tif"
    tif_b = outpath + os.sep + os.path.basename(outpath) + "_B.tif"
    print(os.path.basename(tif_a), os.path.basename(tif_b))
    return [tif_a, tif_b]
...
class Calcu(object):
    def __init__(self, filepath):
        ...
        filepath = r"G:\SDGsat\pearlRiverEstuary"  # 数据路径
    ...
    def run_main(self):
        ...
        for num, zipfile_temp in enumerate(self.zipfiles):
            infiles = decomp(zipfile_temp)
            # infiles = [
            #     r"E:\SDGSAT-1\SCS-SNR\KX10_MII_20220327_E112.58_N14.96_202200105739_L4A" + os.sep +
            #     "KX10_MII_20220327_E112.58_N14.96_202200105739_L4A_B_SNR.tif"]
            for i in range(infiles.__len__()):
                starttime = datetime.datetime.now()
                print("Processing:" + str(num + 1) + "/" + str(
                    self.zipfiles.__len__() * 2) + " image: " + os.path.basename(infiles[i]))
                # 转为WGS84
                print("preprocessing: WGS84, clip...")
                infile_temp1 = preprocessing.convert_coordinates(infiles[i])
                if any([self.north, self.south, self.west, self.east]):
                    infile_temp1 = preprocessing.clip(infile_temp1,
                                                      south=self.south, north=self.north, west=self.west, east=self.east)
                    if infile_temp1 is None:
                        continue
                self.infile = infile_temp1
                # 二. 获取影像相关的信息
    ...
    def lr(self):
        ...
        airmass1 = 1 / np.cos(self.sza_upscale * np.pi / 180) + 1 / np.cos(self.vza_upscale * np.pi / 180)
        # 7.氧气校正：
        ...
        for band_num in range(self.bands.size):
            lr[:, :, band_num] = interpolate.griddata((self.lat_upscale.flatten(), self.lon_upscale.flatten()),
                                                      ...
                                                      method='nearest')
        # etime = datetime.datetime.now()
        # print("====Total time for rayleigh: {} minutes======".format(((etime - stime).seconds / 60)))
        return lr
    ...
    def create_new_tif(self, infile, outfile, bands, datatype=gdal.GDT_Int16):
...
if __name__ == '__main__':
    Calcu(filepath=r"E:\SDGSAT-1\test").run_main()
