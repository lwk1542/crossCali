# -*- coding: utf-8 -*-
"""
@Time    : 2023/10/1 13:25
@FileName: cross_calibration_sdgsat1miiV2.py
...
class SimulationLtoa(object):
    def __init__(self, sensorid_ref: str, target_file: str, ref_bands: list, farther_dir: str):
        """
        Args:
        ...
        self.sensor_alt = 550
        self.sensorID_ref_bands = ref_bands
    ...
    def run_main(self) -> list[str]:
        ...
        # 参考文件
        ref_files = get_filelist(self.file_index, path=self.ref_filedir, mode='all')
        if ref_files.__len__() == 0:
            print("没有参考的文件")
            return []
        outfile_list = []
        for num_j, ref_file in enumerate(ref_files):
            # 读取目标影像
            # 二. 获取影像相关的信息
            self.block_size_rows = self.rows()  # 一次读取原始影像的行数
            self.get_img_info(self.target_file)
            # 读取参考影像
            self.get_refimg_info(infile=ref_file)

            for self.block_num, (data, self.lon, self.lat) in enumerate(self.data_Iterator):
                # "读取的依然是一整个文件"
                self.Lt = data.transpose(1, 2, 0)
                # 取共同区域
                ...
                print("***********参考影像{}".format(os.path.basename(ref_file)))
                # 1. 气象数据加载/计算气体吸收
                ...
                self.output_info = {"tar_file": self.target_file, "ref_file": ref_file}
    ...
    def pre_parameters(self, date_str):
        ...
        if self.sensorID_ref == "s3b_olci":
            self.file_index = ['S3B', "____" + date_str, '_seadas_rrs.hdf']
        elif self.sensorID_ref == "s3a_olci":
            self.file_index = ['S3A', "____" + date_str, '_seadas_rrs.hdf']
        else:
            pass
    ...
    def meteor_para_and_gas_absorb(self):
        ...
        tg_sensor_no2, tg_solar_no2 = gas_transmittance.transmittance_NO2(kno2=self.k_no2, sza=self.sza,
                                                                          vza=self.vza,
                                                                          strat_no2=strat_no2, trop_no2=trop_no2)
        tg_sensor_co2, tg_solar_co2 = gas_transmittance.transmittance_co2(t_co2=self.t_co2, sza=self.sza,
                                                                          vza=self.vza)
        tg_sensor_h2o, tg_solar_h2o = gas_transmittance.transmittance_h2o(water_vapor=water_vapor, sza=self.sza,
                                                                          vza=self.vza,
                                                                          zia_table=self.Zia_table)
    ...
    def output(self) -> str:
        ...
        outfile = self.outdir + os.sep + id_ + "_" + \
