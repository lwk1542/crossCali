...
from sharepy import get_filelist, read_aerosol_seadas, spatial_limit
import mask
...
class CrossCalibration(object):
    def __init__(self, file_list: list, path:str=r'G:\SDGsat\calibration\sea\202303',sensorID_ref:str="s3b_olci"):
        self.path = path
        ...
        self.targetfiles=file_list
        self.sensor_alt = 550
        fileSPA = self.path + os.sep + "spatialLimit.txt"
        if os.path.exists(fileSPA):
            "如果每个文件的空间范围不一样"
            self.spa_lim = spatial_limit.spatial_limit(txtfile=fileSPA)
        ...
        self.outdir=self.path + os.sep + "result"
        self.ref_filedir = self.path + os.sep + 'reference'  # 数据路径  idx, *args, path=None
        self.tar_filedir = self.path + os.sep + 'target'
        if self.sensorID_ref == "s3b_olci":
            self.file_index = 'S3B'
            self.sensorID_ref_bands = [0, 2, 3, 5, 7, 15, 16]
        elif self.sensorID_ref == "s3a_olci":
            self.file_index = 'S3A'
            self.sensorID_ref_bands = [0, 2, 3, 5, 7, 15, 16]
        else:
            pass
    ...
    def run_main(self):
        ...
        current_datetime_ = current_datetime.strftime("T%Y%m%d-%H%M%S")
        logfile = open(self.tar_filedir + os.sep + "crosscalibration_logfile_"+current_datetime_+".txt", "a")
        ...
        # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        crossfile=[]
        for num, filepath in enumerate(self.targetfiles):
            # 匹配参考文件和待定标文件
            print('cross calibration Processing:' + str(num + 1) + '/' + str(self.targetfiles.__len__()) + ' image：'+os.path.basename(filepath))
            logfile.write("\n" + "=======第{}个目标文件：开始=============".format(num))
            logfile.write("\n" + "target file:{}".format(os.path.basename(filepath)))
            if self.sensorID == "sdgsat1_mii":
                from sensor.sdgsat1mii import read_img,extra_info
            self.lut_path, self.nirs_num, self.nirl_num, year, month, day, doy, date_str2 = extra_info(file=filepath)

            # 参考文件
            if (self.sensorID_ref=="s3b_olci") or (self.sensorID_ref=="s3a_olci"):
                self.files = get_filelist.get_filelist(self.file_index, "____" + date_str2, '_seadas_rrs.hdf', path=self.ref_filedir, mode='all')
            else:
                print("没有参考的文件")
                return 3

            for num_j, ref_file in enumerate(self.files):
                print("~~~~~~~~~~~~~~~交叉定标，第{}个匹配文件~~~~~~~~~~~~~~~：开始".format(num_j))
                print("匹配的文件:", os.path.basename(filepath), os.path.basename(ref_file))
                ...
                if self.spatial_limit_index == 1:
                    ...
                    print("south:{},north:{},west:{},east:{}".format(self.south, self.north, self.west, self.east))
                logfile.write(
                ...
                imagery_dict = read_img.Read(filepath=filepath, south=self.south, north=self.north, west=self.west,
                                             east=self.east, resize=0.03).run_main()  # 降分辨率
                ...
                id_temp1 = os.path.basename(imagery_dict["tar_file"]).split("_", -1)
                id_ = "_".join(id_temp1[0:5])
                outfile = imagery_dict["out_dir"] + os.sep + id_ + "_" + \
                ...
                if type(result) is not str:
                    ...
                    print("生成的交叉定标文件：{}".format(os.path.basename(result)))
                    crossfile.append(result)
                logfile = self.log_result(result, logfile)
                print("！！！！！！！！交叉定标，第{}个匹配文件！！！！！！！！：完成".format(num_j))
        logfile.close()
        return crossfile
    ...
    def simulate_ltoa(self, imagery_dict, ref_file):
        ...
        # LTOA
        Lt_simu = (tLf + Lr + TLg + La / fsol + tLw)*tg_sol*tg_sen

        id_temp1 = os.path.basename(imagery_dict["tar_file"]).split("_", -1)
        id = "_".join(id_temp1[0:5])

        outfile = imagery_dict["out_dir"] + os.sep + id + "_" + \
                  os.path.splitext(imagery_dict["ref_file"])[0][0:13]+\
                  os.path.splitext(imagery_dict["ref_file"])[0][63:94]+"_crossCalibration.h5"
        ...
        geomask = mask.mask(sza=sza, saa=saa, vza=vza, vaa=vaa, sza_ref=sza_ref, saa_ref=saa_ref, vza_ref=vza_ref,
                            vaa_ref=vaa_ref)
