...
def exec():
    ...
    sensor_alt = 780
    # 数据路径
    ref_filedir = r'G:\high_quality\H1A_26images\calibration_h1a\MODIS'
    tar_filedir = r'G:\high_quality\H1A_26images\calibration_h1a\OCT'

    target_files = general.get_filelistv2('H1A_RICH_OCT_L1B_', '_10.h5', path=tar_filedir, mode='all')
    # ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

    for num, target_file in enumerate(target_files):
        # 匹配参考文件和待定标文件

        print('Processing:' + str(num + 1) + '/' + str(target_files.__len__()) + ' image')
        # 0.1 获取参考传感器的时间

        ref_sensorID = os.path.basename(target_file)[0:3]
        if any([ref_sensorID == 'H1A', ref_sensorID == 'H1B', ref_sensorID == 'H1C', ref_sensorID == 'H1D']):
            date_str = os.path.basename(target_file)[17:25]
            year, month, day = date_str[0:4], date_str[4:6], date_str[6:]
            date = datetime.datetime.strptime(date_str, '%Y%m%d')
            date_str2 = date.strftime('%Y%j')  # 年积日日期 20001001
            doy=date_str2[4:]

        else:
            continue

        ref_files = general.get_filelistv2('MOD.1KM.L2A.', date_str2, '.hdf', path=ref_filedir, mode='all')

        # 如果没有任何参考传感器的文件，则跳过本次循环
        if ref_files.__len__() == 0:
            continue
        ...
        for ref_file in ref_files:
            ...
            if any([sensorID == 'H1A', sensorID == 'H1B', sensorID == 'H1C']):
                ...
                if out1.__len__() == 0:
                    ...
                    continue
                sza1, vza1, saa1, vaa1, lat1, lon1, dn, bands, Fo, Tau_r, koz, kno2, tco2, zia_table, aw, bbw \
                    = out1
            ...
            Rrs, F0, delta, La_nirl, Fo_nirl, lat2, lon2, sza2, vza2, saa2, vaa2, \
            aermod_up_idx, aermod_low_idx = readfile.read_aerosol_info_seadas(infile=ref_file)
            rhoa_nirl = La_nirl * np.pi * fsol / (Fo_nirl * np.cos(sza2 * np.pi / 180))
            ...
            sza = sza1[up1:low1, left1:right1]
            vza = vza1[up1:low1, left1:right1]
            saa = saa1[up1:low1, left1:right1]
            vaa = vaa1[up1:low1, left1:right1]
            lat = lat1[up1:low1, left1:right1]
            lon = lon1[up1:low1, left1:right1]
            dn = dn[up1:low1, left1:right1]

            # 参考传感器
            loc2 = np.where(
                (south_area < lat2) & (lat2 < north_area) & (west_area < lon2) & (lon2 < east_area))
            if loc2[0].size < 10 or loc2[1].size < 10:
                continue
            up2, low2, left2, right2 = np.min(loc2[0]), np.max(loc2[0]), np.min(loc2[1]), np.max(loc2[1])
            lat_ref = lat2[up2:low2, left2:right2]
            lon_ref = lon2[up2:low2, left2:right2]
            ...
            sza_ref = sza2[up2:low2, left2:right2]
            vza_ref = vza2[up2:low2, left2:right2]
            saa_ref = saa2[up2:low2, left2:right2]
            vaa_ref = vaa2[up2:low2, left2:right2]
            ...
            # 3. 瑞利光学厚度的气压校正 /* Pressure correct the Rayleigh optical thickness */
            ...
            # 8. 水体贡献
            # Terra MODIS对COCTS波段 [412, 443, 469, 488, 531, 551, 555, 645, 667, 678, 748, 859, 869, 1240, 1640, 2130]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--target_sensor_image_path", type=str, default=r'G:\high_quality\H1A_26images\calibration_h1a\OCT',
                        help="a path of calibrated imagery")
    parser.add_argument("--target_image_strID", type=list, default=['H1A_RICH_OCT_L1B_', '_10.h5'],
                        help="a list consists of several string, which are IDs of imagery name")
    parser.add_argument("--reference_sensor_image_path", type=str, default=r'G:\high_quality\manual_25_40_118_128',
                        help="a path of reference imagery")
    # parser.add_argument("--target_image_strID", type=list, default=['MOD.1KM.L2A.', '.hdf'],
    #                     help="a list consists of several string, which are IDs of imagery name")
    parser.add_argument("--south", type=float, default=20, help="unit：degree")
    parser.add_argument("--north", type=float, default=10, help="unit：degree")
    parser.add_argument("--east", type=float, default=128, help="unit：degree")
    parser.add_argument("--west", type=float, default=118, help="unit：degree")
