...
@file: atmospheric_correctionV1.0.py
@time: 2022/1/9 18:00
@desc: 将信息读取全部转移到readfile.py文件中
"""
...
if __name__ == '__main__':
    ...
    for num, infile in enumerate(files):
        ...
        [lut_path, sensor_info, image_info] = readfile.get_info(infile=infile, mode="ac", north=north, south=south,
                                                                west=west, east=east)
        ...
        while last_iter.min() == 0:  # last_iter是一个数组，任何一个像元没有达到停止迭代条件，均需要继续迭代
            ...
            if nonzeroNIR == "want_nirLw":
                fqfile=r"D:\researchProject_lwk\atmospheric_correction\oceancolor_acnirv2\share\common/morel_fq.h5"
                rhown_nir = get_rhown_nir.get_rhown_eval(num_443=num_443, num_555=num_555, num_670=num_670,
                                                         nirs_num=nirs_num, nirl_num=nirl_num, Rrs=Rrs, chl=chl,
                                                         aw=aw, bbw=bbw, bands=bands,
                                                         sza=sza, vza=vza, saa=saa, vaa=vaa,
                                                         fqfile=fqfile)
                for ib in range(nirs_num, nirl_num + 1):
                    tLw_nir[:, :, ib] = rhown_nir[:, :, ib] / np.pi * Fo[ib] * mu0[:, :, 0] * \
        ...
        brdf_mod = brdfmodel.BRDF(vza=vza, sza=sza, vaa=vaa, saa=saa, bands=bands, F0=FoBAR, chl=chl, nlw=nLw,
                                  b443=num_443,  b490=num_490, b520=num_520, b555=num_555, b670=num_670,
                                  foqopt="FOQMOREL",
                                  ws=winds_peed,
                                  fqfile=fqfile)
