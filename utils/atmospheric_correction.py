...
# from oceancolorACnirV2.share.seawifs.l2gen import aerosol_radV2, atmosphericParameter, gas_transmittance, \
#     get_rhown_nir
# from oceancolorACnirV2.share.seawifs.l2gen import whitecap_rad, general, get_chl, \
...
if __name__ == '__main__':
    ...
    filepath = r'D:\test\ac3'  # 数据路径
    files = general.get_filelistv2('H1C_OPER_OCT_L1A', "10.h5", path=filepath, mode='all')
    # +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    for num, infile in enumerate(files):
        ...
        # 根据文件名确定查找表路径
        lut_path= readfile.lut_select(infile=infile)
        ...
        if any([sensorID == 'H1A', sensorID == 'H1B', sensorID == 'H1C']):
            ...
            num_443,num_490,num_520,num_555,num_670,nirs_num,nirl_num,nwvis = 1,2,3,4,5,6,7,6
            red=num_670
        ...
        # 海洋1b星对近红外波段做平滑
        if sensorID == "H1B":
            mask1=Ltemp[:, :, -2]/Ltemp[:, :, -2]
            mask2 = Ltemp[:, :, -1] / Ltemp[:, :, -1]
            # mask=mask1*mask2
            arr_temp=ffill(Ltemp[:, :, -2])
            arr_temp = bfill(arr_temp)
            Ltemp[:, :, -2] = ndimage.uniform_filter(arr_temp, size=8)*mask1
            del arr_temp
            arr_temp = ffill(Ltemp[:, :, -1])
            arr_temp = bfill(arr_temp)
            Ltemp[:, :, -1] = ndimage.uniform_filter(arr_temp, size=8)*mask2
            del arr_temp
        ...
        while last_iter.min() == 0:    # last_iter是一个数组，任何一个像元没有达到停止迭代条件，均需要继续迭代
            ...
            if nonzeroNIR == "want_nirLw":
                rhown_nir = get_rhown_nir.get_rhown_eval(num_443=num_443, num_555=num_555, num_670=num_670,
                                                         nirs_num=nirs_num, fqfile=fqfile, bands=bands,
