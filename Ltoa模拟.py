...
@desc: 指定aot865 geom rrs和气溶胶模型，模拟Ltoa
"""
...
from atmosphericCorrection.oceancolorACnirV2.share.seawifs.l2gen import aerosol_rad_phd, gas_transmittance
...
class simula(object):
    ...
    def __init__(self):
        self.figpath = "F:\calilbrationSite\Figure"
        self.dataPath = r"D:\researchProject\定标检验中的时-空-谱问题\spatial"
        self.size = 8
    ...
    def generate_data(self):
        ...
        self.Rrs = np.arange(0.0001, 0.03, 0.0002)
        self.aot865 = np.arange(0., 0.25, 0.01)
        ...
        variableNum = list(itertools.product(self.Rrs, self.aot865, vza, sza, reaa))
        ...
        aot865_input = np.array(aot_temp).reshape(1, -1, 1)
        ...
        return aot865_input, rrs_input, np.array(sza_input).reshape(1, -1), \
               np.array(vza_input).reshape(1, -1), np.array(reaa_input).reshape(1, -1)

    def rt(self, aot865, rrs_w, sza, vza, reaa):
        ...
        for aeroindex in range(0, 12):
            modnum = np.ones_like(winds_peed, dtype=int) * aeroindex  # 27~36
            aerosol = aerosol_rad_phd.fixd_aer_aot(aot=aot865, modnum=modnum, sza=sza, vza=vza, reaa=reaa,
                                                   aerosol_lut_filepath=aerosol_lut_filepath, taur=Tau_r,
                                                   pressure=pressure)
            taua_mod_cal, rhoax, t_sensorx, t_solarx = aerosol
            ...
            self.df = pd.DataFrame(out,
                                   columns=[str(int(i)) + "nm" for i in bands] + ["aot", "rrs_w", "sza", "vza", "reaa"],
                                   index=None)
            insitu_file = r"F:\cali_spatial_vari\443nm\敏感性分析/" + "simulatedLtoa_aeromodel_" + models[
                aeroindex] + ".xlsx"
            self.df.to_excel(insitu_file, sheet_name="Ltoa_simulation_" + sensorID, index=True)
